...
const NAV_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ─── TOPBAR ─── */
  .nb-topbar {
    background: linear-gradient(135deg, #0f2d22 0%, #1a4a38 50%, #133829 100%);
    height: 72px;
    display: flex;
    align-items: center;
    padding: 0 48px;
    ...
    border-bottom: 1px solid rgba(184,152,72,0.2);
    flex-wrap: wrap;
    gap: 12px;
  }
  ...
  .nb-topbar::before {
    ...
    inset: 0;
    background:
      radial-gradient(ellipse 600px 200px at 50% -60%, rgba(184,152,72,0.12) 0%, transparent 70%),
      repeating-linear-gradient(
        -45deg,
        transparent,
        transparent 30px,
        rgba(184,152,72,0.015) 30px,
        rgba(184,152,72,0.015) 31px
      );
    pointer-events: none;
  }
  .nb-topbar::after {
  ...
  .nb-logo:hover { transform: translateY(-2px) scale(1.02); }
  ...
  .nb-logo:hover .nb-logo-emblem img {
    filter: brightness(0) invert(1) sepia(1) saturate(4) hue-rotate(5deg) brightness(1.2);
  }
  /* Fallback SVG logo */
  .nb-logo-svg {
    width: 52px; height: 52px;
  ...
  .nb-logo:hover .nb-logo-svg { transform: scale(1.08) rotate(-3deg); }

  .nb-logo-wordmark { display: flex; flex-direction: column; gap: 3px; }
  .nb-logo-name {
    ...
    line-height: 1;
    background: linear-gradient(135deg, #ffffff 0%, var(--gold3) 100%);
  ...
  .nb-info-group::before {
    content: '';
    position: absolute; inset: 0;
    background: linear-gradient(135deg, rgba(184,152,72,0.08), rgba(184,152,72,0.02));
    opacity: 0;
    transition: opacity 0.3s;
  }
  .nb-info-group:hover { border-color: rgba(184,152,72,0.4); transform: translateY(-1px); box-shadow: 0 4px 20px rgba(184,152,72,0.12); }
  .nb-info-group:hover::before { opacity: 1; }
  .nb-info-icon {
    color: var(--gold); display: flex; align-items: center; flex-shrink: 0;
    transition: transform 0.35s cubic-bezier(0.34,1.56,0.64,1), color 0.25s;
    position: relative; z-index: 1;
  }
  .nb-info-group:hover .nb-info-icon { transform: scale(1.15) rotate(5deg); color: var(--gold2); }
  .nb-info-main {
  ...
  /* ─── TOP ACTIONS ─── */
  .nb-actions { display: flex; align-items: stretch; height: 100%; position: relative; z-index: 1; margin-left: 12px; }
  @media (max-width: 1024px) {
    .nb-actions {
      margin-left: 8px;
    }
  ...
  .nb-action {
    ...
    gap: 4px; width: 64px; height: 100%;
    color: rgba(245,240,232,0.5); cursor: pointer; text-decoration: none;
  ...
  .nb-action::before {
    content: '';
    position: absolute; inset: 0;
    background: linear-gradient(180deg, rgba(184,152,72,0.08), rgba(184,152,72,0.04));
  ...
  .nb-action:hover { color: var(--gold2); }
  .nb-action:hover::before { transform: scaleY(1); }
  ...
  .nb-action:hover svg { transform: translateY(-3px) scale(1.15); }
  .nb-action-lbl { font-size: 7.5px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; position: relative; z-index: 1; }
  @media (max-width: 768px) {
    .nb-action-lbl {
      font-size: 6.5px;
      letter-spacing: 1.5px;
  ...
  .nb-action.is-menu.open { color: var(--gold2); }
  ...
  .nb-ham.open span:nth-child(2) { opacity: 0; width: 0; transform: scaleX(0); }
  ...
  .nb-mainbar {
    ...
    padding: 0 48px;
    border-bottom: 1px solid rgba(26,74,56,0.08);
    animation: mainbarReveal 0.65s cubic-bezier(0.22,1,0.36,1) 0.1s both;
    position: relative;
  ...
  .nb-mainbar::after {
    ...
    height: 3px;
    background: linear-gradient(90deg, transparent, var(--deep) 20%, var(--mid) 50%, var(--deep) 80%, transparent);
    opacity: 0.08;
  }
  @keyframes mainbarReveal {
    from { opacity: 0; transform: translateY(-12px) rotateX(-8deg); }
    to   { opacity: 1; transform: translateY(0) rotateX(0deg); }
  }

  /* scrolled states */
  .nb-root.scrolled .nb-topbar { height: 62px; box-shadow: 0 4px 30px rgba(0,0,0,0.3); transition: all 0.35s ease; }
  .nb-root.scrolled .nb-mainbar { height: 52px; box-shadow: 0 6px 30px rgba(0,0,0,0.1); transition: all 0.35s ease; }

  /* ─── NAV LINKS ─── */
  .nb-nav { display: flex; align-items: stretch; list-style: none; flex: 1; flex-wrap: nowrap; gap: 0; }
  @media (max-width: 768px) {
    .nb-nav {
      gap: 4px;
    }
  ...
  .nb-nav-link::before {
    ...
    background: linear-gradient(135deg, rgba(26,74,56,0.06), rgba(26,74,56,0.02));
    border-radius: 2px;
  ...
  .nb-nav-link::after {
    ...
    box-shadow: 0 0 8px rgba(26,74,56,0.4);
  }
  .nb-nav-link:hover, .nb-nav-link.active { color: var(--deep); }
  .nb-nav-link:hover::before { transform: scaleX(1) scaleY(1); opacity: 1; }
  .nb-nav-link:hover::after, .nb-nav-link.active::after { transform: scaleX(1); }

  .nb-star {
    color: var(--gold); font-size: 7px; opacity: 0.7;
    transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1), color 0.25s, opacity 0.25s;
    position: relative; z-index: 1;
  }
  @media (max-width: 768px) {
    .nb-star {
  ...
  .nb-nav-link:hover .nb-star, .nb-nav-link.active .nb-star {
  ...
  .nb-dropdown li {
    opacity: 0; transform: translateX(-10px) translateZ(-10px);
    transition: opacity 0.22s ease, transform 0.22s cubic-bezier(0.4,0,0.2,1);
    position: relative;
  }
  .nb-nav-item:hover .nb-dropdown li { opacity: 1; transform: translateX(0) translateZ(0); }
  ...
  .nb-dropdown a {
    ...
    transition: color 0.2s, padding-left 0.25s cubic-bezier(0.4,0,0.2,1),
                background 0.2s, border-left-color 0.2s;
  }
  @media (max-width: 768px) {
    .nb-dropdown a {
  ...
  .nb-dropdown a::before {
    content: '';
    position: absolute; inset: 0;
    background: linear-gradient(135deg, rgba(26,74,56,0.05), transparent);
    transform: translateX(-100%);
    transition: transform 0.3s cubic-bezier(0.4,0,0.2,1);
  }
  .nb-dropdown a:hover {
    color: var(--deep); padding-left: 28px;
    border-left-color: var(--gold);
  }
  .nb-dropdown a:hover::before { transform: translateX(0); }

  .nb-dropdown-icon {
    color: var(--gold); font-size: 10px;
    opacity: 0; transform: translateX(-6px) scale(0.8);
  ...
  .nb-dropdown a:hover .nb-dropdown-icon { opacity: 1; transform: translateX(0) scale(1); }
  ...
  /* divider lines in dropdown */
  .nb-dropdown li.nb-divider {
    height: 1px; background: rgba(26,74,56,0.06); margin: 4px 18px; padding: 0;
  ...
  .nb-dropdown li.has-sub > .nb-sub-menu {
    ...
    transition: opacity 0.25s, transform 0.25s cubic-bezier(0.4,0,0.2,1), visibility 0.25s;
    pointer-events: none;
    perspective: 600px;
  }
  .nb-dropdown li.has-sub:hover > .nb-sub-menu {
    opacity: 1; visibility: visible;
  ...
  .nb-sub-menu a:hover {
    color: var(--deep); padding-left: 28px;
    background: rgba(26,74,56,0.04); border-left-color: var(--gold);
  }

  /* ─── MEGA DROPDOWN ─── */
  .nb-mega-dropdown {
    position: absolute; top: calc(100% + 1px); left: -180px;
    width: 780px;
    background: #ffffff;
    border-top: 3px solid var(--deep);
  ...
  .nb-nav-item:hover .nb-mega-dropdown {
    opacity: 1; visibility: visible;
    transform: translateY(0) rotateX(0deg);
    pointer-events: auto;
  }
  ...
  .nb-mega-col a {
    ...
    transition: color 0.2s, padding-left 0.22s cubic-bezier(0.4,0,0.2,1), border-left-color 0.2s;
  }
  ...
  .nb-cta {
    display: flex; align-items: center; gap: 10px;
    margin: auto 0 auto 20px; padding: 0 22px; height: 38px;
    border: none;
    background: linear-gradient(135deg, var(--deep) 0%, var(--mid) 100%);
    color: #ffffff;
    font-size: 9px; font-weight: 700; letter-spacing: 2.8px; text-transform: uppercase;
    cursor: pointer; position: relative; overflow: hidden;
    transition: all 0.32s cubic-bezier(0.4,0,0.2,1);
    border-radius: 1px; font-family: 'Outfit', sans-serif;
    flex-shrink: 0; white-space: nowrap;
    box-shadow: 0 4px 16px rgba(26,74,56,0.25), inset 0 1px 0 rgba(255,255,255,0.1);
  }
  .nb-cta::before {
    content: '';
    position: absolute; inset: 0;
  ...
  .nb-cta:hover {
    color: var(--deeper);
    box-shadow: 0 8px 32px rgba(184,152,72,0.4), inset 0 1px 0 rgba(255,255,255,0.2);
  ...
  .nb-search-overlay::before {
    ...
    position: absolute; inset: 0;
    background:
      radial-gradient(ellipse 800px 500px at 50% 40%, rgba(184,152,72,0.08) 0%, transparent 70%),
      repeating-linear-gradient(-45deg, transparent, transparent 40px, rgba(184,152,72,0.02) 40px, rgba(184,152,72,0.02) 41px);
    pointer-events: none;
  }
  .nb-search-inner {
  ...
  .nb-search-overlay.open .nb-search-inner {
  ...
  .nb-mega {
    ...
    background: linear-gradient(180deg, #0f2d22 0%, #1a4a38 100%);
    overflow: hidden; max-height: 0; opacity: 0;
    transition: max-height 0.5s cubic-bezier(0.4,0,0.2,1), opacity 0.35s ease;
    z-index: 999;
    border-top: 1px solid rgba(184,152,72,0.2);
  }
  .nb-mega.open { max-height: 500px; opacity: 1; }
  .nb-mega::before {
    content: '';
    position: absolute; inset: 0;
    background: repeating-linear-gradient(-45deg, transparent, transparent 30px, rgba(184,152,72,0.015) 30px, rgba(184,152,72,0.015) 31px);
    pointer-events: none;
  }
  .nb-mega-list { list-style: none; padding: 16px 0; border-bottom: 1px solid rgba(184,152,72,0.1); position: relative; z-index: 1; }
  .nb-mega-item {
    opacity: 0; transform: translateX(-20px) translateZ(-10px);
    transition: opacity 0.32s ease, transform 0.32s cubic-bezier(0.4,0,0.2,1);
  }
  .nb-mega.open .nb-mega-item { opacity: 1; transform: translateX(0) translateZ(0); }
  ...
  .nb-mega-link::before {
    content: '';
    position: absolute; inset: 0;
    background: linear-gradient(135deg, rgba(184,152,72,0.08), transparent);
    transform: translateX(-100%);
    transition: transform 0.3s cubic-bezier(0.4,0,0.2,1);
  }
  .nb-mega-link:hover {
    color: var(--gold2); padding-left: 64px;
    border-left-color: var(--gold);
  }
  .nb-mega-link:hover::before { transform: translateX(0); }
  .nb-mega-arrow {
  ...
  .nb-mega-footer {
    ...
    padding: 18px 48px;
    border-top: 1px solid rgba(184,152,72,0.1);
  ...
  .nb-mega-footer-copy {
    ...
    color: rgba(245,240,232,0.28); letter-spacing: 1.5px;
  }
  .nb-mega-socials { display: flex; gap: 20px; }
  .nb-mega-social {
    display: flex; align-items: center; gap: 6px;
    color: rgba(245,240,232,0.32); font-size: 9px; font-weight: 600;
    letter-spacing: 2px; text-transform: uppercase;
    text-decoration: none; cursor: pointer;
    transition: color 0.22s;
  }
  .nb-mega-social:hover { color: var(--gold); }
  ...
  @keyframes pulseBadge {
    0%, 100% { transform: scale(1); box-shadow: 0 0 6px var(--gold2); }
    50% { transform: scale(1.3); box-shadow: 0 0 12px var(--gold2); }
  }

  /* ─── CURSOR GLOW on topbar ─── */
  .nb-topbar .nb-cursor-glow {
    position: absolute; width: 200px; height: 200px;
    border-radius: 50%;
    background: radial-gradient(circle, rgba(184,152,72,0.06) 0%, transparent 70%);
  ...
  @keyframes navItemPop {
    ...
    60% { transform: scale(1.02) rotateX(1deg); }
    100% { transform: scale(1) rotateX(0deg); }
  }
  .nb-nav-item:hover .nb-nav-link {
    animation: navItemPop 0.35s cubic-bezier(0.34,1.56,0.64,1) both;
  }
`;

/* ── SVG Logo fallback ── */
const AminLawLogo = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="46" height="46" fill="currentColor">
    <g>
      <ellipse cx="28" cy="100" rx="7" ry="13" transform="rotate(-30 28 100)"/>
      <ellipse cx="20" cy="80" rx="6" ry="11" transform="rotate(-50 20 80)"/>
      <ellipse cx="22" cy="60" rx="6" ry="10" transform="rotate(-70 22 60)"/>
      <ellipse cx="32" cy="42" rx="5" ry="9" transform="rotate(-85 32 42)"/>
      <ellipse cx="46" cy="30" rx="5" ry="8" transform="rotate(-100 46 30)"/>
      <ellipse cx="35" cy="118" rx="6" ry="11" transform="rotate(-15 35 118)"/>
      <ellipse cx="28" cy="135" rx="6" ry="10" transform="rotate(0 28 135)"/>
    </g>
    <g>
      <ellipse cx="172" cy="100" rx="7" ry="13" transform="rotate(30 172 100)"/>
      <ellipse cx="180" cy="80" rx="6" ry="11" transform="rotate(50 180 80)"/>
      <ellipse cx="178" cy="60" rx="6" ry="10" transform="rotate(70 178 60)"/>
      <ellipse cx="168" cy="42" rx="5" ry="9" transform="rotate(85 168 42)"/>
      <ellipse cx="154" cy="30" rx="5" ry="8" transform="rotate(100 154 30)"/>
      <ellipse cx="165" cy="118" rx="6" ry="11" transform="rotate(15 165 118)"/>
      <ellipse cx="172" cy="135" rx="6" ry="10" transform="rotate(0 172 135)"/>
    </g>
    <circle cx="72" cy="40" r="10"/>
    <path d="M55 95 Q60 65 72 55 Q80 70 80 95 Z"/>
    <circle cx="128" cy="40" r="10"/>
    <path d="M145 95 Q140 65 128 55 Q120 70 120 95 Z"/>
    <path d="M62 78 L100 45 L138 78" stroke="currentColor" strokeWidth="7" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
    <rect x="80" y="95" width="40" height="42" rx="2"/>
    <rect x="88" y="108" width="24" height="29" rx="1" fill="#1a4a38"/>
    <polygon points="100,165 102,171 108,171 103,175 105,181 100,177 95,181 97,175 92,171 98,171" fill="currentColor"/>
  </svg>
);
...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!topbar || !glow) return;
    const onMove = (e: MouseEvent) => {
      const rect = topbar.getBoundingClientRect();
      glow.style.left = `${e.clientX - rect.left}px`;
      glow.style.top = `${e.clientY - rect.top}px`;
    };
    topbar.addEventListener("mousemove", onMove);
    return () => topbar.removeEventListener("mousemove", onMove);
  }, []);
  ...
  return (
    <div className={`nb-root${scrolled ? " scrolled" : ""}`}>
      ...
      {/* ===== SEARCH OVERLAY ===== */}
      <div className={`nb-search-overlay${searchOpen ? " open" : ""}`} onClick={(e) => { if (e.target === e.currentTarget) setSearchOpen(false); }}>
      ...
      {/* ===== TOP BAR ===== */}
      <div className="nb-topbar" ref={topbarRef}>
        <div className="nb-cursor-glow" ref={glowRef} />

        {/* Logo */}
        <a className="nb-logo" href="#">
          <div className="nb-logo-svg" style={{ color: "var(--gold2)" }}>
            <AminLawLogo />
          </div>
          <div className="nb-logo-divider" />
          <div className="nb-logo-wordmark">
            <span className="nb-logo-name">Amin Law</span>
            <span className="nb-logo-tagline">Associates · Est. 2005</span>
          </div>
        </a>
      ...
      <div className="nb-mainbar">
        <ul className="nb-nav">
          {NAV_ITEMS.map(item => (
            <li key={item.label} className="nb-nav-iem">
              <button
                                ...
                                {sub.sub.map(s => (
                                  <li key={s.label}>
                                    <a href="#" onClick={e => e.preventDefault()}>
                                      <span className="nb-dropdown-icon" style={{fontSize:'8px'}}>◆</span>
                                      {s.label}
                                    </a>
                                  </li>
                                ))}
        ...
        <div style={{display:'flex', alignItems:'center', gap:'10px', marginLeft:'auto'}}>
          <button className="nb-cta" style={{
            display:'flex', alignItems:'center', gap:'8px',
            margin:'auto 0', padding:'0 20px', height:'36px',
            background:'transparent', border:'1.5px solid rgba(26,74,56,0.25)',
            color:'var(--deep)', fontSize:'8px', fontWeight:700, letterSpacing:'2.5px', textTransform:'uppercase',
            cursor:'pointer', fontFamily:'Outfit, sans-serif', flexShrink:0,
            transition:'all 0.25s', borderRadius:'1px',
          }}
          onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.borderColor='var(--deep)';(e.currentTarget as HTMLButtonElement).style.background='rgba(26,74,56,0.05)';}}
          onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.borderColor='rgba(26,74,56,0.25)';(e.currentTarget as HTMLButtonElement).style.background='transparent';}}
          >
